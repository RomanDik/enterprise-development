using Library.Domain;
using Library.Domain.Entities;
using Microsoft.EntityFrameworkCore;

namespace Library.Infrastructure.Repositories;

/// <summary>
/// Репозиторий для управления сущностью Reader
/// </summary>
public class ReaderRepository(LibraryDbContext context) : IRepository<Reader, Guid>
{
    private readonly DbSet<Reader> _dbSet = context.Readers;

    /// <summary>
    /// Создание нового читателя
    /// </summary>
    public async Task<Reader> Create(Reader entity)
    {
        await _dbSet.AddAsync(entity);
        await context.SaveChangesAsync();
        return entity;
    }

    /// <summary>
    /// Получение читателя по Id
    /// </summary>
    public async Task<Reader?> Read(Guid entityId)
    {
        return await _dbSet.FirstOrDefaultAsync(r => r.Id == entityId);
    }

    /// <summary>
    /// Получение всего списка читателей
    /// </summary>
    public async Task<IList<Reader>> ReadAll()
    {
        return await _dbSet.AsNoTracking().ToListAsync();
    }

    /// <summary>
    /// Обновление существующего читателя
    /// </summary>
    public async Task<Reader> Update(Reader entity)
    {
        _dbSet.Update(entity);
        await context.SaveChangesAsync();
        return entity;
    }

    /// <summary>
    /// Удаление читателя по Id
    /// </summary>
    public async Task<bool> Delete(Guid entityId)
    {
        var readerToRemove = await _dbSet.FirstOrDefaultAsync(r => r.Id == entityId);

        if (readerToRemove == null)
        {
            return false;
        }

        _dbSet.Remove(readerToRemove);
        await context.SaveChangesAsync();

        return true;
    }
}