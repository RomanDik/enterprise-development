    ...
    where TKey : struct
{
    ...
    public async Task<ActionResult<TDto>> Create(TCreateUpdateDto newDto)
    {
        ...
        {
            logger.LogError("A not found exception happened during {method} method of {controller}: {@exception}", nameof(Get), GetType().Name, ex);
    ...
    public async Task<ActionResult<TDto>> Edit(TKey id, TCreateUpdateDto newDto)
    {
        ...
        {
            logger.LogError("A not found exception happened during {method} method of {controller}: {@exception}", nameof(Edit), GetType().Name, ex);
            return NotFound($"{ex.Message}");
        }
        catch (Exception ex)
        {
    ...
    public async Task<ActionResult<TDto>> Get(TKey id)
    {
        ...
        try
        {
            ...
            return res != null ? Ok(res) : NotFound("Entity not found");
        }
        catch (Exception ex)
        {
